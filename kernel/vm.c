...
void proc_kpagetable(pagetable_t pagetable) {
  ...
  // map kernel text executable and read-only.
  mappages(pagetable, KERNBASE, (uint64)etext - KERNBASE, KERNBASE,
           PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  mappages(pagetable, (uint64)etext, PHYSTOP - (uint64)etext, (uint64)etext,
           PTE_R | PTE_W);
...
int kvmcopy_user(pagetable_t kpagetable, pagetable_t upagetable,
                 uint64 start_va, uint64 end_va) {
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  for (i = PGROUNDUP(start_va); i < end_va; i += PGSIZE) {
...
// unmap user page from kernel page
void kvmunmap_user(pagetable_t kpagetable, uint64 start_va, uint64 end_va) {
  uint64 a = PGROUNDUP(start_va);
  uint64 b = PGROUNDUP(end_va);
  if (a < b) {
    uvmunmap(kpagetable, a, (b - a) / PGSIZE, 0);
  }
}
