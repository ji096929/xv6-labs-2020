...
static void proc_free_kpagetable(struct proc *p);
...
found:
  ...
  if (mappages(p->kpagetable, va, PGSIZE, (uint64)kstack, PTE_R | PTE_W) < 0) {
    kfree(kstack);
    freeproc(p);
    release(&p->lock);
    return 0;
...
void userinit(void) {
  ...
  p->sz = PGSIZE;
  if (kvmcopy_user(p->kpagetable, p->pagetable, 0, p->sz) < 0)
    panic("userinit: kvmcopy_user");
...
int growproc(int n) {
  ...
  if (n > 0) {
    if (sz + n >= PLIC)
      return -1;
    if ((sz = uvmalloc(p->pagetable, p->sz, sz + n)) == 0) {
      return -1;
    }
    if (kvmcopy_user(p->kpagetable, p->pagetable, p->sz, sz) < 0) {
      uvmdealloc(p->pagetable, sz, p->sz);
      return -1;
    }
  } else if (n < 0) {
    sz = uvmdealloc(p->pagetable, p->sz, p->sz + n);
    kvmunmap_user(p->kpagetable, sz, p->sz);
  }
  p->sz = sz;
  return 0;
}
...
int fork(void) {
  ...
  np->sz = p->sz;
  if (kvmcopy_user(np->kpagetable, np->pagetable, 0, np->sz) < 0) {
    freeproc(np);
    release(&np->lock);
    return -1;
  }
